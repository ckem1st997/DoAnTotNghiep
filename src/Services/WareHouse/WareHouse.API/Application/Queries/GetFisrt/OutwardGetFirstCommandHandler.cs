...
namespace WareHouse.API.Application.Queries.GetFisrt
{
    ...
    public class OutwardGetFirstCommandHandler : IRequestHandler<OutwardGetFirstCommand, OutwardDTO>
    {
        ...
        public async Task<OutwardDTO> Handle(OutwardGetFirstCommand request, CancellationToken cancellationToken)
        {
            ...
            if (res != null)
            {
                res.OutwardDetails = (System.Collections.Generic.ICollection<Domain.Entity.OutwardDetail>)await _repositoryDetail.GetByAsync(x => x.OutwardId.Equals(request.Id) && x.OnDelete == false);
                if (res.OutwardDetails != null)
                {
                    foreach (var item in res.OutwardDetails)
                    {
                        item.SerialWareHouses = (System.Collections.Generic.ICollection<Domain.Entity.SerialWareHouse>)await _repositorySeri.GetByAsync(x => x.OutwardDetailId.Equals(item.Id) && x.OnDelete == false);
